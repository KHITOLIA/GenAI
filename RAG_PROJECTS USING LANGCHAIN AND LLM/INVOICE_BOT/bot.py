...
if uploaded_file is not None:
    ...
    if not os.listdir(vector_dir):  # If DB for this file doesn't exist
        try:
            ...
            st.success('✅ Loading the documents')
            text_chunks = create_chunks(extracted_data=documents)
            st.success("✅ Creating the text chunks")
            vectorstore = create_embeddings(text_chunks=text_chunks)
            st.success("✅ Creating the vector database")
            vectorstore.save_local(vector_dir)
            st.success(f"✅ Vector Database saved successfully at {vector_dir}!")   
        except Exception as e:
            st.error(f"❌ Error processing the file: {e}")
    else:
        st.info("ℹ️ Vector database already exists for this file. Skipping creation.")
...
if st.button('🔍 Retrieve Context'):
    if user_query.strip() == "":
        ...
        st.warning("⚠️ Upload a document before querying.")
    else:
        try:
            st.info("🔎 Searching from Vector Database...")    
            ...
            st.error(f"❌ Failed to retrieve information: {str(e)}")

# Stop button to clean up
if st.button("🛑 Stop it's processing"):
    try:
